...
Lexer.prototype = {
  ...
  lookahead: function(n){
	...
	decl: function () {
		var cap;
		if (cap = /^let *(\w[\w\d]+) *= */.exec(this.input)) {
			if (this.indentLvl > 0) {
		...
		if (cap = /^([ \n]*,?[ \n]*)\"/.exec(this.input)) {
			...
			for (var i = 1; i < this.input.length; i++) {
				if (this.input[i] === '\"' && this.input[i - 1] !== '\\') {
					var tok = this.tok('string', this.input.substr(1, i - 1).replace(/\\\"/g, '\"'));
					this.consume(i + 1);
					return tok;
				}
			}
		...
		if (cap = /^([ \n]*,?[ \n]*)\{/.exec(this.input)) {
			...
			for (var i = 1; i < this.input.length; i++) {
				if (this.input[i] === '{' && this.input[i - 1] !== '\\') {
					...
					if (--hooks === 0) {
						var tok = this.tok('json', this.input.substr(0, i + 1));
						this.consume(i + 1);
						return tok;
					}
				}
