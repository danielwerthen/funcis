...
Runner.prototype = {
	...
	assembleContext: function (exStack) {
		var key = exStack[exStack.length - 1].pos.join('');
		if (!this.contexts[key])
			return this.contexts[key] = new Context(this, exStack, key);
		else
...
function evalArg(str, stack, constants) {
	...
	while (cap = re.exec(str)) {
		if (!val) {
			val = stack[cap[1]] || (constants[cap[1]] ? constants[cap[1]].val : null);
		}
		else {
			val = val[cap[1]];
		}
